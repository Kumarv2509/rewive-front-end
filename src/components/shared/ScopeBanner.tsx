...
import { PERSONA_LABEL } from '../../screens/CommandCenter/personas';
...
export function ScopeBanner() {
  const { persona, rolesInScope, reports } = useEffectiveLens();
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', margin: '2px 0 14px', fontSize: 12, color: 'var(--ink-3)' }}>
      <span className="pill indigo">{PERSONA_LABEL[persona]}</span>
      {hierarchyOn ? (
        <>
          <span>+ their team:</span>
          {reports.map((r) => (
            <span key={r} className="pill gray">{PERSONA_LABEL[r]}</span>
          ))}
          <span>— everything owned along this reporting line.</span>
        </>
      ) : (
        <span>
