...
// When the picture carries P&L-line nodes, they form a DuPont tier between the
// intents and the mandates: intent ← P&L line ← mandate ← sense, top to bottom.
const COL_W = 250;
const ROW_H = 150;
const TARGET_Y = 0;
const PL_Y = 185;
const KPI_START_Y = 210;
const KPI_START_Y_WITH_PL = 400;
const NODE_W = 210;
...
export function computePositions(brain: KpiBrain): Record<string, { x: number; y: number }> {
  ...
  const plLines = brain.nodes.filter((n) => n.kind === 'pl_line' && n.status !== 'declined');
  const kpiStartY = plLines.length > 0 ? KPI_START_Y_WITH_PL : KPI_START_Y;
  ...
  for (const key of streamKeys) {
    ...
    let row = 0;
    for (const n of kpis) { pos[n.id] = { x, y: kpiStartY + row * ROW_H }; row++; }
    row += 0.5; // small gap before the driver(s)
    for (const n of drivers) { pos[n.id] = { x, y: kpiStartY + row * ROW_H }; row++; }
  }
  ...
  // The DuPont tier: P&L lines span a row between intents and mandates, in
  // statement order (their seed order), so the row reads like the P&L.
  if (plLines.length > 0) {
    const plGap = fullWidth / plLines.length;
    plLines.forEach((n, i) => { pos[n.id] = { x: plGap * (i + 0.5) - NODE_W / 2, y: PL_Y }; });
  }
...
export function toFlowEdges(edges: BrainEdge[], litEdgeIds: Set<string> | null): Edge[] {
  return edges.map((e) => {
    ...
    const color = proposed ? 'var(--accent)' : lit ? 'var(--accent-deep)' : 'color-mix(in srgb, var(--ink) 28%, transparent)';
    return {
      id: e.id,
      source: e.source,
      target: e.target,
