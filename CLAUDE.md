...
Everything is industry-parameterized: signing in at `/login` picks the organization — and with it the operating context (FMCG, Healthcare, Hypermarket, or Manufacturing) — and every screen swaps its content accordingly. No production backend exists — a mock Express server in `mock-server/` implements the exact REST contract the frontend expects, with in-memory stateful mutations.
...
**Tenancy (demo-grade):** the SaaS front door is `/login` — a split-view organization sign-in (brand panel left, card right) in **two steps: find your organization, then that org's branded sign-in**. The front door never lists tenants (a multi-tenant product doesn't show one customer the others): step 1 resolves the org from what the user types — name, workspace id, or work email — via `findTenants` in `src/tenants.ts`, which distinguishes found / ambiguous / unknown; a `?org=<tenantId>` deep link (invite-style; nothing in the app mints one today — landing CTAs are plain `/login` links) skips straight to step 2, and "Switch organization" (TopNav + palette) lands on step 1 with nothing preselected. `src/tenants.ts` registers the tenants (Americana Foods → fmcg, Medcare UAE → healthcare, GulfMart Hypermarkets → hypermarket, Gulf Precision Industries → manufacturing) with flat brand accents, demo email domains, and session helpers (`rewive.tenant` in localStorage). Signing in sets tenant + industry + persona lens ("Sign in as" role → each team lands in its own view); all app routes sit behind `RequireTenant` in `App.tsx`. The industry choice stays authoritative: `getActiveTenant()` re-derives the tenant if the industry is switched in-app (Operating Picture) or the session predates tenancy. The top nav shows the signed-in org chip + "Switch organization". **The auth seam (ARCH-GTM-001 P1.1/P1.2, demo-grade credentials, production-grade shape):** sign-in mints an HS256 JWT (`mock-server/auth.js`, `src/api/auth.ts`; any password works). Server-side, `authMiddleware` validates **JWT-shaped bearers only** (the cron secret and ingest keys are opaque single strings and pass through untouched) and signed claims outrank `?industry=` everywhere context is resolved (`v4Industry` + the org-profile route). Client-side the token is the authority: `getAuthClaims()` in `client.ts` decodes it (self-pruning on expiry), `getActiveTenant()` is claims-first with `rewive.tenant`/`rewive.industry` demoted to caches synced from claims, and `useSetIndustry` **re-mints the token for the tenant owning the new industry** when one is held — without that, an in-app industry switch silently reverts because the server ignores the query param. Tokenless sessions fall back to legacy demo behavior end-to-end. Entra OIDC later replaces the issuer (dev secret → JWKS), not the middleware contract, claim names, or client plumbing.
